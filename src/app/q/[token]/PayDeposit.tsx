...
// DEMO deposit checkout — clearly labeled, accepts anything, charges nothing.
export default function PayDeposit({
  ...
  async function pay() {
    ...
    try {
      const res = await fetch(`/api/q/${token}/pay-deposit`, { method: "POST" });
    ...
    <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-zinc-200">
      ...
      <div className="mt-3 space-y-2">
        <input
          ...
          className="w-full rounded-xl border border-zinc-300 px-3 py-3 placeholder:text-zinc-500"
        />
        <div className="grid grid-cols-2 gap-2">
          <input
            ...
            placeholder="MM/YY"
            className="rounded-xl border border-zinc-300 px-3 py-3 placeholder:text-zinc-500"
          />
          <input
            inputMode="numeric"
            autoComplete="off"
